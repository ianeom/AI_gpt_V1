...
# ==============================================================================
# 🧮 100% 精準獎金計算機 (直接內嵌在同個檔案，不需新建檔案)
# ==============================================================================
def calculate_pc_bonus(numbers_of_people: int):
    qdv_per_pack = 77
    profit_per_pack = 20
    total_qdv = numbers_of_people * qdv_per_pack
    
    # 1. 計算優選客戶利潤
    total_profit = numbers_of_people * profit_per_pack
    ...
    total_referral_bonus = total_qdv * referral_rate
    grand_total = total_profit + total_referral_bonus
    
    return {
        "總人數": numbers_of_people,
        "總QDV": total_qdv,
        "優選客戶利潤": f"${total_profit} 美元",
        "客戶介紹金": f"${total_referral_bonus:.2f} 美元",
        "當週預估總獎金": f"${grand_total:.2f} 美元"
    }
...
def get_prompt_and_respond(user_id: str, user_message: str) -> str:
    ...
    # 🟡 第二階段完全體任務內容
        ...
        return (
            ...
            "2. 成功複製出 2 位「二星經理」核心幹部，完成二星大盤複製者，請向平哥教練領取暗號，進軍一星董事！"
        )
    ...
    try:
        # ⚡ 徹底移除任何可能的空格，確保 chat.completions 連續不中斷
